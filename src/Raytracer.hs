...
-- |Trace single ray segment against a scene and return closest intersection if there is one
traceRay :: Scene -> Maybe Entity -> RaySegment -> Maybe (Intersection Entity)
traceRay scene toSkip raySeg =
    foldl closest Nothing . map (intersect raySeg) . skip toSkip $ scEntities scene
  where closest x0 Nothing = x0
        closest Nothing x@(Just (Hit t _ _ _ _ _)) = if t >= 0 then x else Nothing
        closest x0@(Just (Hit t0 _ _ _ _ entity0)) x@(Just (Hit t _ _ _ _ entity)) =
          if entity /= entity0 && t >= 0 && t < t0 then x else x0

        skip Nothing  = id
        skip (Just e) = filter (/= e)

-- |Shoot many rays using "shooter" function
shootMany :: (t1 -> t -> (LightIntensity, t1)) ->
  Int -> t1 -> LightIntensity -> Maybe t -> (LightIntensity, t1)
shootMany _ _ g0 envLightIntensity Nothing  = (envLightIntensity, g0)
shootMany shooter cnt g0 _ (Just geomHit)   = (avgLightIntensity, g'')
  where avgLightIntensity = averageIntensity samples
        (samples, g'') = iterate (\(xs, g) -> first (:xs) $ shooter g geomHit) ([], g0) !! cnt

-- |Trace single path of rays
pathTrace :: RandomGen gen => gen -> Scene -> RaySegment -> (LightIntensity, gen)
pathTrace gen scene@(Scene _ light envLight settings) =
    pathTrace' maxDepth Nothing gen
  where maxDepth = rsPathMaxDepth settings 

        -- |Helper separation so we could reuse scene and initial conditions across recursive calls
        pathTrace' 0 _ g _                      = (envLight, g)
        pathTrace' levelsLeft prevHit g0 raySeg = ((+) <$> lightIntensity <*> giIntensity, g''')
          where (RaySeg (Ray (_, shootDir), _)) = raySeg
                ...
                lightSamplesCount = if isCameraRay then rsLightSamplesCount settings else 1

                -- Trace shadow rays and GI rays
                (lightIntensity, g'') = shootMany bounce2light lightSamplesCount g0  envLight geomHit     -- shadow rays shoot first
                (giIntensity, g''')   = shootMany bounce2GI    giSamplesCount    g'' envLight geomHit     -- gi rays shoot

                -- |Shadow rays tracing
                bounce2light g hit@(Hit _ ipoint _ _ _ entity) = (reflectedLight, g')
                  where (shadowRaySeg, g')  = shadowRay g light ipoint
                        RaySeg (Ray (_, dir2light), _) = shadowRaySeg

                        reflectedLight =
                          case traceRay scene (Just entity) shadowRaySeg of
                ...
                -- |GI tracing
                bounce2GI g hit@(Hit { isectEntity = entity }) = (exRadiance, gLast)
                  where (inRadiance, gLast) = pathTrace' (levelsLeft-1) (Just entity) g' (RaySeg (nextRay, farthestDistance))
                ...
                evalHitBRDF hit irradiance irrDir = radiance
                  where Mat brdf     = enMaterial $ isectEntity hit
...
imageSample :: (RandomGen gen, Camera cam) => gen -> Scene -> cam -> UnitSpace -> (LightIntensity, gen)
...
raytrace gen scene camera = Image width height raw
  where raw    = fromList $ concatMap fromPixel pxList
        ...
        mapLightIntensity (V3 r g b) = PixelRGB8 (f2w r) (f2w g) (f2w b)
          where f2w f = truncate (min 1 (((f /~ lumen) * exposure) ** invGamma) * 255)

        -- |Raytraced pixels
        pixels gen' = map pixel $ zip (generators gen') (screenCoords sensor)
          where pixel (g, (x,y)) = mapLightIntensity . fst $ imageSample g scene camera $ toScreenSpace sensor x y
...
screenCoords (Sensor (width, height, _, _)) =
